...
# def compute_style(model, ref_dicts):
...
def build_tts_model(device, config, text_aligner, pitch_extractor, plbert):
...
def LFinference(model, tokens, s_prev, noise, alpha=0.7, diffusion_steps=5, embedding_scale=1):
    tokens.insert(0, 0)
...
def main():
    ...
    model = build_tts_model(device, config, text_aligner, pitch_extractor, plbert)
    ...
    with AudioFile(
        ...
        for i, token in enumerate(tokenize_text(global_phonemizer, text)):
            ...
            wav, s_prev = LFinference(model, token, s_prev, noise, alpha=0.5, diffusion_steps=10, embedding_scale=2.5)
            f.write(wav)
